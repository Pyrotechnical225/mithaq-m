...
import { deleteExampleUsers, seedExampleImams, seedExampleUsers } from "@/lib/admin.functions";
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Populate the app with fictional example profiles and imams so you can test matching, the
        imam finder, and the admin flows without waiting for real users.
      </p>
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-4">
        ...
        <button
          disabled={busy}
          onClick={() => run("Seed imams", doSeedImams)}
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            disabled={busy}
            onClick={() => run("Seed users", doSeedUsers)}
            className="rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90 disabled:opacity-60"
          >
            Seed example users
          </button>
