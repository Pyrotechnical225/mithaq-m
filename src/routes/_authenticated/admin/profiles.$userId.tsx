...
function EditProfile() {
  ...
  const load = () => fetchDetail({ data: { user_id: userId } }).then((d) => {
    setDetail(d);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-baseline justify-between">
        <div>
          <Link to="/admin/profiles" className="text-xs text-muted-foreground hover:text-foreground">← All profiles</Link>
          <h1 className="mt-1 text-3xl text-foreground">{displayName || "Untitled profile"}</h1>
          <p className="text-sm text-muted-foreground">
            {detail.auth?.email} · joined {detail.auth?.created_at ? new Date(detail.auth.created_at).toLocaleDateString() : "—"}
            {detail.auth?.email_confirmed_at ? " · verified" : " · unverified"}
          </p>
        </div>
        <button onClick={remove} className="rounded-full border border-destructive/40 px-4 py-1.5 text-xs text-destructive hover:bg-destructive/10">Delete profile</button>
      </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="mt-1 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm" />
          </label>
          <label className="block">
            <span className="text-xs uppercase tracking-widest text-muted-foreground">Contact email</span>
            <input value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} className="mt-1 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm" />
          </label>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          {(["hidden", "discoverable", "paused"] as const).map((v) => (
            <label key={v} className={`cursor-pointer rounded-full border px-4 py-1.5 text-sm ${visibility === v ? "border-primary bg-primary/10 text-primary" : "border-border text-foreground"}`}>
              <input type="radio" className="hidden" checked={visibility === v} onChange={() => setVisibility(v)} />
              {v}
            </label>
          ))}
        </div>
